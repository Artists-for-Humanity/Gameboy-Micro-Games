export default class MainMenu extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "bg1",
      new URL("gameAssets/bgframe.png", import.meta.url).href
    );
    this.load.image(
      "bg2",
      new URL("gameAssets/framemetalplates.png", import.meta.url).href
    );
    this.load.image(
      "play",
      new URL("gameAssets/play.png", import.meta.url).href
    );
    this.load.image(
      "finger",
  ...
  update() {
    if (Phaser.Input.Keyboard.JustDown(this.up)) {
      this.currentBottonIndex === 0
        ? (this.currentBottonIndex = 2)
        : (this.currentBottonIndex -= 1);
    ...
    if (Phaser.Input.Keyboard.JustDown(this.space)) {
      if (this.currentBottonIndex == 0) {
        this.scene.start(
          this.listOfGames[Phaser.Math.Between(0, this.listOfGames.length)]
        );
      }
