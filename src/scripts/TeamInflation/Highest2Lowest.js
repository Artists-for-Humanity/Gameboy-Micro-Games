...
export default class MicroGame31 extends Phaser.Scene { 
    ...
    update() {
        if(!this.gameOver){
            ...
            if(Phaser.Input.Keyboard.JustDown(this.left)){this.movePointer("l")}
            if(Phaser.Input.Keyboard.JustDown(this.right)){this.movePointer("r")}
            if(Phaser.Input.Keyboard.JustDown(this.up)){this.movePointer("u")}
            if(Phaser.Input.Keyboard.JustDown(this.down)){this.movePointer("d")}
    ...
    fillBox(i, x, y){
        ...
        if(this.opcode[i] !== 0){
            ...
            switch(smallcheck){
                case 1: this.box[i].setScale(.8, .5) 
                        break
                case 2: this.box[i].setScale(.6, .5) 
                        break
                default: break
            }
        }
    ...
    movePointer(dir){
        ...
        switch(this.selected){
            ...
            case 0: 
                if(dir === "r"){this.selected = 1}
                else if(dir === "d"){this.selected = 2}
                break
            // moving from top right
            case 1:
                if(dir === "l"){this.selected = 0}
                else if(dir === "d"){this.selected = 2}
                break
            // moving from center
            case 2:
                if(dir === "l"){this.selected = 3}
                else if(dir === "r"){this.selected = 1}
                else if(dir === "u"){this.selected = 0}
                else if(dir === "d"){this.selected = 4}
                break
            // moving from bottom left
            case 3:
                if(dir === "r"){this.selected = 4}
                else if(dir === "u"){this.selected = 2}
                break
